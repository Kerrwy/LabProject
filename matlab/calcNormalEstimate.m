...
if(v == 0)
    ...
    n = round(atan2d(vy,vx)/45)+5; % {1,2,3,...9} index in local position vector 定义 atan2(y,x) 返回点(x,y)与x轴正向的夹角；[局部速度不变性]
    ...
    t_est = sqrt(nx^2+ny^2)/v+t; % distance between pixel centers divided by velocity 像素之间的距离除以速度
end
